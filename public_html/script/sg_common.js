...
function zoomImg(imgID)
{
   ...
   if (obj = document.getElementById('overlay'))
   {
      ...
      var strImage = "<div class=\"background\"></div>"
                   ...
                   + "<div class=\"legend\">" + imgLegend + ((imgCopyr) ? "<span class=\"copyright\">" + imgCopyr + "</span>" : "") + "</div>"
                   + "</div>";
      
      obj.innerHTML = strImage;
      zWndHeight    = Math.min(zWndHeight,Sitepark.Common.Geometry.getViewportHeight());
      
      intWidth  = Math.max(Sitepark.Common.Geometry.getViewportWidth(),document.getElementsByTagName('html')[0].offsetWidth,document.getElementsByTagName('body')[0].offsetWidth, Sitepark.Common.Geometry.getDocumentWidth());
      intHeight = Math.max(Sitepark.Common.Geometry.getViewportHeight(),document.getElementsByTagName('html')[0].offsetHeight,document.getElementsByTagName('body')[0].offsetHeight, Sitepark.Common.Geometry.getDocumentHeight());
